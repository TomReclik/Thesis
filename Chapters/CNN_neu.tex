...
A single neuron is capable of distinguishing between two classes that are linearly separable, when more classes exist or the classes are not linearly separable more neurons and more difficult arrangements are necessary. In feedforward neural networks the neurons are arranged in a layered fashion were each layer consists of multiple neurons. With this layered structure problems can be solved that are impossible for a single neuron like the famous XOR problem, that lead to a drop in research about neural networks. This problem can be solved by using a layered structure with two neurons in its first layer and a single neuron in its single layer assigning a label. This process is illustrated in figure \ref{fig:XOR}. \\
...
Another, in modern architectures, often employed layer type is a batch normalization layer. In their paper \cite{Szegedy2015} the Szegedy et. al argue that the shift in the distribution of each layer during training leads to a diminished learning rate, called an internal covariance shift. In order to reduce this shift, the input $x_i$ of a batch $B$ is firstly normalized such that it has zero mean and a unity variance
\begin{equation}
\hat{x}_i = \frac{x_i-\mu_B}{\sqrt{\sigma_B^2+\epsilon}}
\end{equation}
where $\mu_B$ is the expectation value of the batch, $\sigma_B$ is the standard deviation of the batch, and $\epsilon$ is a regularization parameter preventing a division by zero. Since this normalization may change what a network is capable of learning, this is then rescaled
\begin{equation}
