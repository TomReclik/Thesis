...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{ClassificationSetting.pdf}
...
\centering
  ...
  \label{fig:Neuron}
  \includegraphics[width=\linewidth]{Neuron_3.pdf}
...
\subsection{CNN}\label{sec:DiscreteConvolutions}
...
%The basis for a ANNs with a special internal structure so called convolutional neural networks are convolutions that will be explained in the following section. \\
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Cathedral_gray.png}
  %\caption{Original image gray scale image.}
  %\label{fig:sub1}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Cathedral_Sobel.png}
...
%\centering
%\subcaptionbox{}%
%  [.3\linewidth]{\includegraphics[width=0.35\linewidth]{In-Situ-Tracking.png}}
%\centering
%\subcaptionbox{}
%  [.67\linewidth]{\includegraphics[width=0.55\linewidth]{CombinedHeatmapSearchWindow.png}}
...
Such a network topology overcomes the shortcomings of a fully connected network. Firstly the number of trainable parameters is significantly reduced as the weight matrix of each neuron is only sparsely occupied and the weights between neurons in each feature map are restricted to the same values. Furthermore, the internal structure of the input space is taken into account in each layer through the usage of convolutions. \\
...
The softmax function, can be understood as a "softened" version of the arg max function. It is defined by
\begin{equation}
...
% It retains the ordering relation between the outputs of the neurons, falls into the interval $(0,1)$ 
%
%The output of the last layer before the application of an activation func
%
%The last layer is tasked with assigning a class label to the sample presented to the network. The output of the last layer before an activation function has been applied, $z_j$, can take any value. In order to extract a class label one possibility is to take the index of the element with the largest value, i.e. $c = \text{arg max}_j z_j$, with $c$ being the assigned class and $j=0,\dots,k-1$ being the index of the neurons . Since this function is not differentiable, it can not be used as the activation function of the last layer. As described earlier, the output of the last layer is used in order to define a distance between the output of the network and the true class labels, via the cost function. The output of the network should therefore be close to $1$ for the neuron corresponding to the correct class. One function fulfilling this condition is the softmax function. As its name suggests it is a "softened" version of the max function. It is defined by
%\begin{equation}
%\sigma(z)_j = \frac{e^{z_j}}{\sum_i e^{z_i}}
%\end{equation}
%As can be seen from this definition, the values that the softmax function can take fall in the interval $(0,1)$ and sum to one. This enables a rather intuitive interpretation of the output of the network as a probability like value. 
%
%The output of the last layer is decisive for the class assigned to a sample processed by the network. The simplest option to transform the output, which consists of a vector $z \in \mathbb{R}^k$ is to take the maximum value of that vector. The problem with this approach is that this activation function is not differentiable, which is necessary for the backpropagation algorithm to work. Another option for the activation function of the last layer is the softmax function. As its name suggests, this function is a "softened" version of the max function. It is defined by 
%As can be seen from this equation, the ordering of the output of the last layer remains unchanged. Furthermore, the values of this function fall into the interval $[0,1]$ and sum up to one. This enables a probabilistic interpretation of the output, e.g. for a three class problem an output $z=(0.2,0.2,0.6)$ can be interpreted as the sample being class $1$ or $2$ with a probability of $0.2$ or class $3$ with a probability of $0.6$. As the network is trained, the networks weights are optimized in such a manner, that probability of an object belonging to class $i$ is maximized. After the network has been trained, the classes can be assigned by either using the class with the highest probability, or introduce a threshold. Classification then is performed by taking the maximum probability and also check whether this probability exceeds the chosen threshold. This can be summarized as follows
%\begin{equation}
%c_j = \begin{cases}
%\text{arg max}_j\text{ } \sigma(z)_j & \text{if}\quad \text{max } \sigma(z)_j>\theta \\
%-1 & \text{else}
...
%
%Giving a rule for the construction of a CNN architecture is 
%
%Due to the diversity of CNNs, that score high metrics on benchmark tests and applications, and the lack of a deep understanding the reasons behind components increasing i
%
%Due to the high diversity of CNNs in application and the lack of a true underlying understanding of the interplay of different components, finding plan for how a CNNs architecture should be constructed is impossible. Rather 
%
%On the highest level, a CNN can be separated into two parts, feature extraction through the subsequent application of convolutions, and classification through the application of a fully connected neural network in the end. 
%
%As well performing networks differ in their specifics wildly, some general rules can be found. 
%\begin{itemize}
%\item \textbf{Regularization}: After each convolution layer, use either dropout or batch normalization as a regularization method.
%\item \textbf{Pooling}: Use pooling after the consecutive application of convolutional layers, where the exact number of convolutional layers before pooling varies.
%\end{itemize}
...
% listed in the following with decreasing levels of detail.
...
A networks performance can then be investigated by inspecting the different metrics, plotted against the classification rate, shown for the accuracy of an optimal, a random, and a suboptimal classifier in figure \ref{fig:PURvsEFF}.
...
%\subsubsection{Purity vs Efficiency}
